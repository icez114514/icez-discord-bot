...
撲克牌預設使用 `casino_assets/honkai` 的崩壞3rd角色牌組（52 張一般牌＋1 張 Joker）與統一牌背，完整角色／服裝對照見 manifest.json，瀏覽圖集見 gallery.html。
新牌素材缺少或無法解碼時，退回 `casino_assets/classic` 的經典牌面；來源與生成提示詞見各牌組目錄。
保留自訂 cards/<ID>.png 與 back.png 覆蓋介面；內建牌面等比例縮放、不裁切，素材只在載入快取時讀取。
牌桌仍為 1200×800；高解析角色原圖只在預載時縮放，操作期間使用已快取的小尺寸牌面。
