...
公開 /賭場 → 21 點、18 豆仔或牌九撲克 → 基本下注與倍率 → 開局。
牌九撲克採七張前二後五與全萬用 Joker，操作、固定 House Way 與驗證見 [牌九撲克](paigow.md)。
...
「遊戲篩選」輸入 dice、blackjack、paigow 或其他已記錄遊戲識別，留空查全部。
...
撲克牌預設使用 `casino_assets/embossed` 的白色壓印牌組（52 張一般牌＋1 張 Joker）與統一牌背；純白底只顯示中央大點數、花色，四角不放小標。完整牌表見 manifest.json，瀏覽圖集見 gallery.html。
新牌素材缺少或無法解碼時，退回 `casino_assets/classic` 的經典牌面；來源與重建方式見各牌組目錄。
保留自訂 cards/<ID>.png 與 back.png 覆蓋介面；內建牌面等比例縮放、不裁切，素材只在載入快取時讀取。
牌桌仍為 1200×800；高解析牌面只在預載時縮放，操作期間使用已快取的小尺寸牌面。
