...
def authenticate(db, token):
    ...
    row = db.execute('SELECT user_id FROM sessions WHERE token_hash=? AND revoked=0', (digest(token),)).fetchone()
    if row is None:
...
def present(db, user, now):
    return bool(db.execute('SELECT 1 FROM presence p JOIN sessions s ON s.token_hash=p.session_hash WHERE s.user_id=? AND s.revoked=0 AND p.until_at>?', (user, now)).fetchone())
...
class Sessions(ABC):
    ...
    async def account_for_session(self, token, now=None):
        ...
        def operation(db):
            ...
            return {**account_view(db, user), 'subsidy': subsidy_status(db, user, time.time() if now is None else now)}
    ...
    async def presence(self, token, connection_id, action, now=None):
        ...
        def operation(db):
            user = authenticate(db, token)
            existing = db.execute('SELECT * FROM presence WHERE connection_id=?', (connection_id,)).fetchone()
            if existing and existing['session_hash'] != digest(token):
                raise Conflict('connection_owner_mismatch')
            if action == 'enter':
                db.execute('INSERT INTO presence(connection_id,session_hash,until_at) VALUES(?,?,?) ON CONFLICT(connection_id) DO UPDATE SET until_at=excluded.until_at', (connection_id, digest(token), now+150))
                db.execute('UPDATE accounts SET presence_revision=presence_revision+1 WHERE user_id=?', (user,))
            elif action in ('heartbeat', 'disconnect', 'leave'):
                if not existing:
                    raise Conflict('connection_not_open')
                until = 0 if action == 'leave' else now + (120 if action == 'disconnect' else 150)
                db.execute('UPDATE presence SET until_at=? WHERE connection_id=?', (until, connection_id))
            else:
    ...
    async def oauth_begin(self, now=None):
        ...
        def operation(db):
            ...
            db.execute('INSERT INTO oauth_states VALUES(?,?,?)', (digest(state), digest(browser), now+600))
        await self.run(operation)
    ...
    async def oauth_consume(self, state, browser, now=None):
        ...
        now = time.time() if now is None else now
        def operation(db):
            row = db.execute('SELECT * FROM oauth_states WHERE state_hash=? AND browser_hash=? AND expires_at>?', (digest(state), digest(browser), now)).fetchone()
            if not row:
