...
async def serve(config):
    ...
    def server(app, port):
        return uvicorn.Server(uvicorn.Config(app, host='127.0.0.1', port=port, workers=1, access_log=False, proxy_headers=False, log_level='warning', ws_max_size=4096, lifespan='off'))
    ...
    async with public.router.lifespan_context(public):
        ...
        tasks = [asyncio.create_task(server.serve()) for server in (public_server, internal_server)]
        try:
...
def main():
    ...
    try:
        ...
        if args.command == 'check':
            print('Configuration valid; SQLite ' + sqlite3.sqlite_version + '; one worker; OAuth and Termux not verified by this check')
