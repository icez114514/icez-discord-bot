...
class Config:
    ...
    def validate(self):
        ...
        if not self.secure and (self.environment == 'production' or parsed.hostname not in ('localhost', '127.0.0.1', 'testserver')):
        ...
        if self.environment == 'production' and not self.data_dir.resolve().is_relative_to(Path.home().resolve()):
            errors.append('production database must be under the private user home (Termux HOME)')
        if self.public_port == self.internal_port or any(not 1024 <= port <= 65535 for port in (self.public_port, self.internal_port)):
    ...
    @classmethod
    def from_env(cls):
        return cls(data_dir=Path(os.environ.get('POKER_DATA_DIR', str(Path.home() / '.local/share/icez-poker'))), environment=os.environ.get('POKER_ENV', 'production'), origin=os.environ.get('POKER_ORIGIN', 'https://localhost'), client_id=os.environ.get('POKER_CLIENT_ID', ''), client_secret=os.environ.get('POKER_CLIENT_SECRET', ''), bot_token=os.environ.get('POKER_BOT_TOKEN', ''), guild_id=os.environ.get('POKER_GUILD_ID', ''), reader_token=os.environ.get('POKER_READER_TOKEN', ''), funds_token=os.environ.get('POKER_FUNDS_TOKEN', ''), funds_admins=tuple(filter(None, os.environ.get('POKER_FUNDS_ADMINS', '').split(','))), public_port=int(os.environ.get('POKER_PUBLIC_PORT', '8765')), internal_port=int(os.environ.get('POKER_INTERNAL_PORT', '8766')))