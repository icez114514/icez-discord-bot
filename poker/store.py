...
class Store(Sessions):
    ...
    async def login(self, user_id: str):
        ...
        def operation(db):
            ...
            if created:
                db.execute(
                    "INSERT INTO ledger(command_id,user_id,source,reason,available_delta,settled_delta) VALUES(?,?,?,?,50000,50000)",
                    ("gift:" + user_id, user_id, "initial_gift", "First poker account"),
                )
            db.execute(
                "INSERT INTO sessions(token_hash,user_id,created_at) VALUES(?,?,?)",
                (digest(token), user_id, time.time()),
            )
            db.execute(
                "UPDATE accounts SET login_revision=login_revision+1 WHERE user_id=?",
                (user_id,),
            )
            return token
    ...
    async def command(
        self, command_id: str, kind: str, *, now=None, session=None, **data
    ):
        ...
        fingerprint = json.dumps(
            {"kind": kind, "data": data}, sort_keys=True, separators=(",", ":")
        )
        ...
        def operation(db):
            ...
            try:
                result = {
                    "value": execute(
                        db, command_id, kind, data, time.time() if now is None else now
                    )
                }
    ...
    async def subsidy(self, user_id, now=None):
        ...
        return await self.run(
            lambda db: subsidy_status(db, user_id, time.time() if now is None else now)
        )
                ...
                **dict(row),
                ...
                "events": [
                    ...
                    for event in db.execute(
                        "SELECT event FROM hand_events WHERE hand_id=? ORDER BY id",
                        (hand_id,),
                    )
                ],
                "action_clocks": [
                    ...
                    for clock in db.execute(
                        "SELECT * FROM action_clocks WHERE hand_id=?", (hand_id,)
                    )
                ],
            }
...
def account_view(db, user_id):
    ...
    return {
        ...
        "table": str(row["table_chips"]),
        "in_flight": str(row["in_flight"]),
        "settled": str(row["settled"]),
        "time_bank": row["time_bank"],
        "hand_progress": row["hand_progress"],
    }
