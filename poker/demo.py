...
def demo_app(path, port=8765, eligible=False):
    ...
    config = Config(
        ...
        client_id="123",
        guild_id="456",
        client_secret="test-secret-only-0000",
        bot_token="test-bot-only-000000",
        reader_token="test-read-only-00000",
        funds_token="test-funds-only-0000",
    )
    ...
    async def simulated_authorization(request, call_next):
        ...
        if (
            ...
            await app.state.store.command(
                "demo-opening-loss",
                "adjust",
                user_id="123456789012345678",
                amount="-49000",
                actor="123",
                reason="Explicit TEST demonstration of subsidy",
            )
        return response
...
def main():
    ...
    uvicorn.run(
        demo_app(args.data_dir, args.port, args.eligible),
        host="127.0.0.1",
        port=args.port,
        workers=1,
        access_log=False,
