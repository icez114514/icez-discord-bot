...
class CommandContractTests(unittest.IsolatedAsyncioTestCase):
    async def test_rejected_command_replays_original_rejection_after_balance_changes(self):
        with tempfile.TemporaryDirectory() as directory:
            async with Store(Path(directory)/'poker.db', initialize=True) as store:
                ...
                await store.command('debit', 'adjust', user_id='111', amount='-49000', actor='999', reason='Test debit')
                with self.assertRaises(Conflict):
                    await store.command('help', 'subsidy', user_id='111')
