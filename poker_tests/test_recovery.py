        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            async with Store(path) as store:
                ...
                await store.command(
                    ...
                    user_id="111",
                    amount="-49000",
                    reason="Boundary setup",
                    actor="999",
                )
                boundary = datetime(2026, 9, 15, 20, 0, tzinfo=timezone.utc).timestamp()
                await store.command(
                    "help-before", "subsidy", user_id="111", now=boundary - 1
                )
                await store.command(
                    "spend",
                    "adjust",
                ...
                with self.assertRaises(Conflict):
                    await store.command(
                        "help-same", "subsidy", user_id="111", now=boundary - 0.5
                    )
                await store.command(
                    "help-after", "subsidy", user_id="111", now=boundary
                )
