...
class WebTests(unittest.TestCase):
    def test_oauth_state_membership_origin_and_revoked_websocket(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            config = Config(data_dir=Path(directory), environment='test', origin='http://testserver', client_id='123', client_secret='s'*32, bot_token='b'*32, guild_id='456', reader_token='r'*32, funds_token='f'*32)
            ...
            with TestClient(app) as client:
                ...
                self.assertEqual(client.get('/auth/callback?code=x&state=wrong').status_code, 401)
                self.assertEqual(client.get('/auth/callback', params={'code': 'x', 'state': state}, follow_redirects=False).status_code, 303)
                self.assertEqual(client.get('/api/account').json()['available'], '50000')
                self.assertEqual(client.post('/api/subsidy', json={'command_id': 'one'}).status_code, 403)
                with client.websocket_connect('/ws/presence', headers={'origin': config.origin}) as websocket:
                    self.assertEqual(websocket.receive_json()['type'], 'ready')
                    self.assertEqual(client.post('/auth/logout', headers={'origin': config.origin}).status_code, 200)
