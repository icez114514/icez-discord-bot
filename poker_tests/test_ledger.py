        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            async with Store(path, initialize=True) as store:
                await asyncio.gather(
                    *(store.login("123456789012345678") for _ in range(8))
                )
                self.assertEqual(
                    (await store.account("123456789012345678"))["available"], "50000"
                )
            async with Store(path) as store:
                await store.login("123456789012345678")
                self.assertEqual(
                    (await store.account("123456789012345678"))["settled"], "50000"
                )
