...
class SecurityTests(unittest.TestCase):
    def test_nonmember_and_api_errors_never_log_in(self):
        for code, status, guild_status in (
            (10007, 404, 200),
            (10007, 404, 403),
            (10004, 404, 200),
            (0, 401, 200),
            ...
            with (
                self.subTest(code=code, status=status, guild=guild_status),
                ...
                with TestClient(
                    ...
                    state = parse_qs(urlparse(auth.headers["location"]).query)["state"][
                        0
                    ]
                    response = client.get(
                        "/auth/callback",
                    ...
                    self.assertEqual(
                        ...
                        403
                        if status == 404 and code == 10007 and guild_status == 200
                        else 503,
                    )
                ...
                with TestClient(api, client=("127.0.0.1", 45000)) as internal:
                    ...
                    self.assertEqual(
                        internal.post(
                            ...
                            json=body,
                            headers={
                                "authorization": "Bearer " + config.reader_token,
                                "x-actor-id": "999",
                            },
                        ).status_code,
                        403,
                    ...
                    self.assertEqual(
                        internal.post(
                            ...
                            json=body,
                            headers={
                                "authorization": "Bearer " + config.funds_token,
                                "x-actor-id": "888",
                            },
                        ).status_code,
                        403,
                    ...
                    self.assertEqual(
                        internal.get(
                            "/scans",
                            headers={"authorization": "Bearer " + config.reader_token},
                        ).status_code,
                        200,
                    )
    ...
    def test_production_rejects_test_overrides(self):
        ...
        config = replace(
            self.config(str(Path.home() / ".poker-test-unused")),
